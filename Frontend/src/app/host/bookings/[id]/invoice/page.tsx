...
export default function HostInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      ...
      try {
        ...
        // Menggunakan endpoint getPemesananById untuk host
        const res = await fetch(`http://localhost:5000/api/v1/pemesanan/host/${id}`, {
          headers: {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href="/host/bookings" className="text-primary hover:underline">
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4 sm:px-6 lg:px-8 print:bg-white print:py-0 print:px-0">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Kertas Invoice */}
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden print:shadow-none print:rounded-none border border-gray-200">
          {/* Header */}
          <div className="bg-slate-900 px-8 py-10 text-white flex justify-between items-center print:bg-white print:text-black print:border-b-2 print:border-gray-200">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">StayNest</h1>
              <p className="text-slate-300 mt-1 print:text-gray-500">Salinan Faktur (Host Copy)</p>
            </div>
            <div className="text-right">
              <h2 className="text-2xl font-bold uppercase tracking-wider mb-1 text-slate-100 print:text-black">INVOICE</h2>
              <p className="font-mono text-slate-400 print:text-gray-600">{invoiceNumber}</p>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            {/* Rincian Reservasi */}
            <div className="bg-slate-50 rounded-xl p-6 mb-10 border border-slate-100 print:border-gray-300 print:bg-transparent">
            ...
            <div className="flex justify-end">
              <div className="w-full md:w-1/2">
                ...
                <div className="flex justify-between py-4 mt-2">
                  <span className="text-xl font-bold text-gray-900">Total Pemasukan</span>
                  <span className="text-2xl font-bold text-slate-900">Rp {booking.totalHarga.toLocaleString('id-ID')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-16 pt-8 border-t border-gray-200 text-center text-sm text-gray-500">
              ...
              <p className="mt-1">Terima kasih telah bergabung menjadi Tuan Rumah StayNest!</p>
            </div>
          </div>
        </div>
      </div>
